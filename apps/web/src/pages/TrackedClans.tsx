...
export function TrackedClans() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.addTrackedClan(newTag.trim(), newNote.trim() || undefined);
      setNewTag("");
      setNewNote("");
      loadClans();
    } catch (err) {
  ...
  async function handleRemove(tag: string) {
    try {
      await api.removeTrackedClan(tag);
      loadClans();
    } catch (err) {
  ...
  return (
    <Box>
      ...
      {loading ? (
        ...
        <EmptyState message="No clans being tracked. Add a clan tag above to get started." />
      ) : (
